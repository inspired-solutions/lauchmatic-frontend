...
const AdminDeviceThumbnailAction = {
  addDeviceThumbnail: device => async dispatch => {
    try {
      ...
      dispatch({ type: AdminDeviceThumbnailTypes.ADMIN_ADD_DEVICE_THUMBNAIL, payload: response })
    } catch (error) {
      console.log(error)
    }
  },
  ...
  getDeviceThumbnails: () => async dispatch => {
    try {
      ...
      dispatch({ type: AdminDeviceThumbnailTypes.ADMIN_GET_DEVICE_THUMBNAILS, payload: response })
    } catch (error) {
      console.log(error)
    }
  },
